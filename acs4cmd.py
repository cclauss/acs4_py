...
""",
    ...
    parser.add_option("--dry_run", action="store_true", help="Don't post to server")
    parser.add_option(
        "--port",
        action="store",
    ...
    group = optparse.OptionGroup(
        parser, "Request arguments", "Arguments specific to 'request' actions"
    )
    group.add_option(
        "--start",
        action="store",
    ...
    group = optparse.OptionGroup(
        parser, "Upload arguments", "Arguments specific to 'upload' action"
    )
    group.add_option(
        "--datapath", action="store", help="server data path to use with upload"
    )
    group.add_option(
        "--metadata", action="store", help="xml file of resource metadata - for upload."
    )
    parser.add_option_group(group)
    ...
    for name in request_arg_names:
        ...
        if name in ("distributor", "resource"):
            ...
            parser.add_option(
                "--" + name, action="store", help=name + " argument for request"
            )
    ...
    if action == "queryresourceitems":
        result = acs4.queryresourceitems(
            server,
            opts.password,
            start=opts.start,
            count=opts.count,
        ...
        if len(args) == 3:
            if opts.datapath is not None:
                parser.error(
                    "--datapath (path to remote file)" " and filename both supplied."
                )
            fh = open(args[2])
        elif len(args) == 2:
        ...
        if len(args) != 4:
            parser.error(
                'For "request" action, please supply server,'
        ...
        if not opts.resource or not opts.distributor:
            parser.error(
                "Please supply --resource= and --distributor=" " arguments for mint"
            )
        distinfo = acs4.get_distributor_info(
            server, opts.password, opts.distributor, port=opts.port
        )
        secret = distinfo["sharedSecret"]
        name = distinfo["name"]
        print(
            acs4.mint(server, secret, opts.resource, "enterloan", name, port=opts.port)
        )
    parser.destroy()
